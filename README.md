...
- **A compiled capability artifact**: `CapabilityCompiler.compile()` turns the trace's
  *executed actions* (not the model's raw reasoning) into a `CapabilityDefinition` —
  ...
  — now an exact copy of the artifact generated by the successful Gemini discovery
  in [evidence/20260913T161444Z](evidence/20260913T161444Z/manifest.json).
  The source run ID and SHA-256 bind the artifact to its discovery trace.
